...
func (s *SubscriptionService) UpdateSubscriptionCheckoutStatus(stripCheckoutSessionID string, appID string, status model.SubscriptionCheckoutStatus, customerID *string) error {
	q := s.SQLBuilder.
		Update(s.SQLBuilder.TableName("_portal_subscription_checkout")).
		Set("status", status)

	if customerID != nil {
		q = q.Set("stripe_customer_id", customerID)
	}
